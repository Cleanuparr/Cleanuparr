...
<Note>
`DATABASE_PROVIDER` and the `POSTGRES_*` settings can also be supplied as keys in `cleanuparr.json` (non-Docker installs, see [Port and Base Path (Non-Docker)](/docs/installation/detailed#port-and-base-path-non-docker)) instead of environment variables, exactly like `PORT`, `BIND_ADDRESS` and `BASE_PATH`.
</Note>
