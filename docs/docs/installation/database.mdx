...
2. Run the same image with the `./Cleanuparr migrate-to-postgres` command, the `/config` volume mounted the same way as your normal install, and the `POSTGRES_*` variables pointing at the target database:
