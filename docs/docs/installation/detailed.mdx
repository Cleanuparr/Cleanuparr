...
| `DATABASE_PROVIDER` | sqlite | See [Database](./database) for more info |
| `POSTGRES_HOST` | *(required for postgres)* | See [Database](./database) for more info |
| `POSTGRES_PORT` | 5432 | See [Database](./database) for more info |
| `POSTGRES_USER` | *(required for postgres)* | See [Database](./database) for more info |
| `POSTGRES_PASS` | *(required for postgres)* | See [Database](./database) for more info |
| `POSTGRES_DB` | *(required for postgres)* | See [Database](./database) for more info |
| `POSTGRES_EXTRA_PARAMS` | *(empty)* | See [Database](./database) for more info |
...
Cleanuparr uses SQLite by default, with the database files stored under `/config`. No configuration is needed for this. PostgreSQL is also supported. See the [Database](./database) page for configuration and data migration details.
</div>
