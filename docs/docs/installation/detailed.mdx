...
<Important>
PostgreSQL support is for fresh installs only: there is no migration path from an existing SQLite database to PostgreSQL. Existing SQLite installs are unaffected and can keep using SQLite indefinitely.
</Important>
