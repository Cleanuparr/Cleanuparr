...
| `DATABASE_PROVIDER` | sqlite | See [Database](/docs/installation/database) for more info |
| `POSTGRES_HOST` | *(required for postgres)* | See [Database](/docs/installation/database) for more info |
| `POSTGRES_PORT` | 5432 | See [Database](/docs/installation/database) for more info |
| `POSTGRES_USER` | *(required for postgres)* | See [Database](/docs/installation/database) for more info |
| `POSTGRES_PASS` | *(required for postgres)* | See [Database](/docs/installation/database) for more info |
| `POSTGRES_DB` | *(required for postgres)* | See [Database](/docs/installation/database) for more info |
| `POSTGRES_EXTRA_PARAMS` | *(empty)* | See [Database](/docs/installation/database) for more info |
| `CLEANUPARR_CONFIG_PATH` | *(auto)* | Override the config directory. Available only for non-Docker setups. |
| `CLEANUPARR_LOGS_PATH` | *(config dir)/logs* | Override the log directory. Available only for non-Docker setups. |
...
Cleanuparr uses SQLite by default, with the database files stored under `/config`. No configuration is needed for this. PostgreSQL is also supported. See the [Database](/docs/installation/database) page for configuration and data migration details.
</div>
...
#### Health Checks
Cleanuparr provides health check endpoints for monitoring and container orchestration. For detailed information about available endpoints and configuration, see the [Health Checks documentation](/docs/configuration/health-checks/).
