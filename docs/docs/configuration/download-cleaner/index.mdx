...
<p className={styles.sectionDescription}>
  The Orphaned Files feature scans configured directories for files and folders no longer tracked by any active torrent and moves them to a dedicated orphaned directory. It runs as part of the Download Cleaner job and shares its schedule. Supported for all download clients: qBittorrent, Transmission, Deluge, rTorrent, and uTorrent.
</p>

<p className={styles.sectionDescription}>
  Global settings (filters and purge policy) apply across all clients. Per-client settings (scan paths and orphaned directory) are configured in the <strong>Orphaned Files</strong> accordion inside the per-client section.
</p>
...
Glob patterns for file or folder names that should never be considered orphaned, even if no active torrent claims them. Matching is case-insensitive and applied to the entry name only — not the full path.
...
Minimum age in minutes a file or folder must have before it can be considered orphaned. This protects files that are actively being downloaded or have just finished — they may not yet be registered as a torrent save path.

Set to `0` to disable age filtering (default).

**Example:** Set to `60` to skip any entry modified less than an hour ago.
...
The directory where orphaned files and folders are moved for this download client. Leave empty to log orphaned entries without moving them — useful for a dry-run style audit before committing to actual moves.
