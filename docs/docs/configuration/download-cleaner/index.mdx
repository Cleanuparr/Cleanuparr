...
Tracker domain suffixes used to filter which torrents this rule applies to. A torrent matches if **any** of its tracker domains ends with one of these patterns (case-insensitive). Leave empty to match torrents from any tracker.
...
Supported by **qBittorrent** (tags) and **Transmission** (labels). Silently ignored for Deluge, rTorrent, and µTorrent.
