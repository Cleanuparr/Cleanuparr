...
<SectionTitle>Orphaned Files Settings</SectionTitle>

<p className={styles.sectionDescription}>
  Orphaned files scanning runs as part of the Download Cleaner job. Global settings apply to all clients; per-client settings are configured in the <strong>Orphaned Files</strong> accordion under each download client.
</p>
...
Glob patterns for file or folder names that should never be considered orphaned. Matching is case-insensitive and applies to the entry name only (not the full path).
...
Minimum age in minutes a file or folder must have before it can be considered orphaned. Protects files that are actively being downloaded or just finished. Set to `0` to disable (default).
...
<Warning>
Files deleted by this option are permanently removed from disk.
</Warning>
...
Enable orphaned files scanning for a specific download client. Configure under the **Orphaned Files** accordion in the per-client section.
...
Absolute paths to scan for orphaned files on this download client. Each top-level entry is checked against active torrent save paths. Scan directories must not overlap with those of other clients.
...
Where orphaned files are moved for this download client. Leave empty to log orphaned entries without moving them.
