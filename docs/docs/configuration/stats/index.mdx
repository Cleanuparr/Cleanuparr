...
<Warning>
**`/api/stats` (v1) was removed.** Use **`/api/v2/stats`** instead.
</Warning>
...
- **total**: Total number of events in the timeframe, equal to the sum of `byType`.
- **byType**: Events grouped by event type. Possible types:
  ...
  - `DownloadStopped`: A finished download was stopped in the download client after meeting its seeding goals, instead of being removed.
...
- **total**: Total strikes issued in the timeframe, equal to the sum of `byType`.
...
Downloads deleted by the `Download Cleaner` in the timeframe. Seeding rules with the `Stop` action are counted as `DownloadStopped` events instead.
...
- **arrInstances**: Health of each enabled instance, for every supported application: Sonarr, Radarr, Lidarr, Readarr, Whisparr, Sportarr and LazyLibrarian.
...
<ConfigSection
  title="timeframeHours & generatedAt"
>

- **timeframeHours**: The timeframe the response covers, echoing the requested `hours` after clamping.
...
- `events.total`: Total events in the timeframe.
- `strikes.total`: Strikes issued in the timeframe.
