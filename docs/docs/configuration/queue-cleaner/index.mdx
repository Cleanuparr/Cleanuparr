...
- the *arr runs no import or scan right now, so we don't affect files while they are being moved
- every file the *arr found belongs to the series or movie the download was grabbed for
...
Cleanuparr waits until it has seen the download twice before acting, so the first run after you turn this on changes nothing.
...
Waiting on a busy *arr costs you nothing, since a try means a request that reached the *arr.
You get the notification after the download leaves the *arr queue, so it marks a real import.
...
[Max Strikes](#failed-import-max-strikes) does not gate this, so you can rescue imports with striking switched off. Once the tries run out Cleanuparr hands the download back to the normal failed import path: it collects strikes if you have striking on, and sits there if you do not.
...
The number of times Cleanuparr asks the *arr to import one download before giving up. Default 3.
