...
<Note>
To avoid striking genuine downloads during an internet outage, enable the [Internet Connectivity Check](../general/#internet-connectivity-check) in General settings. When it fails, the Queue Cleaner skips its run so no strikes are accumulated.
</Note>
