...
When enabled, this rule is skipped while the download client's alternate speed limits are active, so intentional bandwidth throttling does not accumulate slow strikes. Both the minimum speed and maximum time checks are suppressed while alternate speed limits are on.
