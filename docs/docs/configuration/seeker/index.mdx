...
When enabled, a download that carries a Queue Cleaner strike does not count toward the Active Download Limit. A slow or stalled download stops holding a slot, so Seeker keeps searching for releases that can still complete.
...
Strikes may reset when a download starts making progress again. Because of this, new downloads might be added before the strikes are reset, causing more downloads than expected to accumulate in the queue.
